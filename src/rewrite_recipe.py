...
from src.schemas import ParsedRecipe, SwapChoice, RewrittenRecipe


SYSTEM = """You rewrite a recipe after ingredient changes.

Rules:
- Preserve the original cooking intent and structure as much as possible.
- Only change ingredients that are explicitly swapped or added by the user.
- For additions, keep the original ingredient and incorporate the new one.
- Update ingredient list and instructions accordingly.
- If a change requires a minor adjustment, incorporate it (and mention in change_log).
...
def rewrite_recipe(parsed: ParsedRecipe, swaps: list[SwapChoice]) -> RewrittenRecipe:
    def format_change(choice: SwapChoice) -> str:
        if choice.action == "add":
            return f"- add {choice.chosen} alongside {choice.original}"
        return f"- swap {choice.original} -> {choice.chosen}"

    swap_lines = "\n".join([format_change(s) for s in swaps])
...
Rewrite the recipe to reflect the changes.
"""
